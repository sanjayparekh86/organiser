...
//                        app.panzoom = data;
        ...
        $("#delete_box").click(function () {
            var $this = $(this);
            $("#popupCDialog").popup("open");
            $(".do_delete_container").off("click").on("click", function () {
                app.deleteFile(app.panzoom.nativeURL);
                $.each(app.containers, function (k, v) {
                    if (v.items.length > 0) {
                        $.each(v.items, function (kk, vv) {
                            if (vv.image != null && vv.image != "") {
...
//                        app.panzoom = data;
    ...
    render_box_data: function () {
        if (localStorage.getItem("action") && localStorage.getItem("action") == "edit") {
            $("#container_edit_options").show();
            $("#save_organiser").hide();
            var all_boxes = $.parseJSON(localStorage.getItem("all_boxes"));
            var selected_id = localStorage.getItem("selected_box");
            app.id = selected_id;
            app.data = all_boxes[app.id];
            var imageData = app.data.image_url;
            app.panzoom.nativeURL = app.data.image_url;
            app.panzoom.imageName = app.data.name;
            var image = document.getElementById('uploaded_image');
            image.src = imageData;
//        console.log(app.data);
            $("#base_name").val(app.data.name);
//         app.apply_zooming();
            setTimeout(function () {
                app.panzoom = $.parseJSON(app.data.data)
                app.apply_zooming();
                var json_data = $.parseJSON(app.data.data)
//                app.panzoom.zoom_val = json_data.zoom_val;
//                app.panzoom.width = json_data.position.width;
//                app.panzoom.height = json_data.height;
//                app.panzoom.img_height = json_data.img_height;
//                app.panzoom.img_width = json_data.img_width;
//                app.panzoom.max_left = json_data.max_left;
//                app.panzoom.max_top = json_data.max_top;
//                app.panzoom.position.top = json_data.position.top;
//                app.panzoom.position.left = json_data.position.left;
//                app.zoom_image(json_data.zoom_val);

                $.each(app.data.container, function (k, v) {
//                    app.containers[app.counter] = v;
                    var new_c_width = app.panzoom.width * v.width / json_data.width;
                    var new_c_height = app.panzoom.height * v.height / json_data.height;
                    var new_c_top = app.panzoom.height * v.top / json_data.height;
                    var new_c_left = app.panzoom.width * v.left / json_data.width;
                    app.counter = v.counter
...
//            app.counter++;
    ...
    apply_box_events: function () {
        // set verticle slider height
//        debugger;
        $("#slider2").attr("data-height", $("#panzoom-parent").outerHeight());
        $(".ui-slider-track").css("height", $("#panzoom-parent").outerHeight() - 50 + "px", "!important");
...
//                $("#items_list").listview("refresh");
    ...
    upload_base_image: function () {
        if (is_local_file(app.panzoom.nativeURL)) {
            ...
            var params = {};
            if (app.panzoom.id != null) {
                params = {"type": "base", id: app.data.id, "old_image": app.data.image};
            }
    ...
    addContainer: function () {
        ...
        if (app.containers[app.counter].id != null) {
            app.containers[app.counter].element.css({
                width: app.containers[app.counter].width + "px",
                height: app.containers[app.counter].height + "px",
                top: app.containers[app.counter].top + "px",
                left: app.containers[app.counter].left + "px"})
        } else {
    ...
    populateContainerList: function (counter) {
        ...
        if ($(".small_container").length > 0) {
            ...
            $.each(app.containers[counter].items, function (k, v) {
                li += `<li class="">
                        <a href="javascript:;"  data-item="${k}">${v.image != null && v.image != "" ? "<img src='" + v.image + "'>" : ""}
                            <h2>${v.name}</h2>
                        </a>
    ...
    setContainerProperties: function () {
        ...
        if ($(".small_container").length > 0) {
            $(".small_container").each(function () {
                ...
                if (app.containers[ele].element == null) {
                    app.containers[ele].element = $(this);
                }
                var pos = app.containers[ele].element.position();
                app.containers[ele].top = pos.top;
                app.containers[ele].left = pos.left;
                app.containers[ele].width = app.containers[ele].element.outerWidth();
                app.containers[ele].height = app.containers[ele].element.outerHeight();
//                    app.containers[ele].is_locked = true
...
//        $("#slider2").slider("disable");
        $(`.small_container`).off("tap dragstart resizestart dragstop resizestop");
        ...
        $(`.small_container`).on("dragstop", function () {
        ...
        $(`.small_container`).on("resizestop", function () {
...
//        $("#slider2").off("slidestop").on("slidestop", function () {
//            var zoom = parseFloat($("#slider2").val());
//            app.zoom_image(zoom);
//        })
    },
    zoom_image: function (zoom) {
...
//        data-split-icon="gear" data-split-theme="a" 
        if (app.containers[counter].items.length > 0) {
            ...
            $.each(app.containers[counter].items, function (k, v) {
                li += `<li class="">
                        <a href="javascript:;" class="item_action" data-item="${k}">${v.image != null && v.image != "" ? "<img src='" + v.image + "'>" : ""}
                            <h2>${v.name}</h2>
                        </a>
                       <!-- <a href="javascript:;" class="item_action" data-item="${k}">Edit</a> -->
                    </li>`;
